...
class CamlBuilder {
  ...
  buildWhere(lbWhere) {
    ...
    return xmlBuilder.buildObject({Where: this._buildCamlCondition(lbWhere)});
  };
  ...
  _buildLogicalCaml(operator, lbConditions, result, index) {
    ...
    if (index === 0) {
      ...
      result[operator].push(firstCondition);
      result[operator].push(secondCondition);
      ...
      const lastElement = result[operator].pop();
      const condition = this._buildCamlCondition(lbConditions[index]);
      const subArray = [lastElement, condition];
      const newElement = {[operator]: subArray};
      result[operator].push(newElement);
      this._buildLogicalCaml(operator, lbConditions, newElement, index + 1);
    }
  };
