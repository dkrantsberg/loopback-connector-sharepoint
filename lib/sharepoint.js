...
SharePointConnector.prototype.all = function (modelName, filter, options, callback) {
  ...
  // } else if (!filter.where && filter.limit && !filter.order) {
  //   getItems = sp.web.lists.getByTitle(this.getSPListTitle(modelName)).items.top(filter.limit).get();
  } else {
    ...
    getItems = sp.web.lists.getByTitle(this.getSPListTitle(modelName)).getItemsByCAMLQuery(camlQuery);
  }
  getItems.then(items => {
