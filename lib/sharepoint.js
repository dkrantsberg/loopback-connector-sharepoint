 ...
 */
SharePointConnector.prototype.update = SharePointConnector.prototype.updateAll = function updateAll(modelName, where, data, options, callback) {
  const self = this;
...
SharePointConnector.prototype.getItemsFiltered = function (modelName, filter, expandLookups) {
  const self = this;
  const camlBuilder = new CamlBuilder(self._models[modelName]);
  ...
  return sp.web.lists.add(listTitle, '')
    ...
    .then((defaultFields) => {
      ...
      for (const prop of Object.keys(model.properties)) {
        const spFieldName = camlBuilder.getSPFieldName(prop);
        // skip fields which already exist
        if (_.includes(existingSPFields, spFieldName)) {
          continue;
        }
        const fieldType = camlBuilder.getSPFieldType(prop);
...
/*!
 * Convert an object containing LB model properties to one containing
 *
 * @param {String} modelName The model name
...
SharePointConnector.prototype.toSPProperties = function (modelName, lbProperties) {
  ...
  for (let propName in lbProperties) {
    const spColumnName = this.getSPColumnName(modelName, propName);
